...
#ifndef LIBDEFIANCE_RULEPARSER_H
#define LIBDEFIANCE_RULEPARSER_H
...
typedef struct {
    ...
    std::string target; // to be used for string match zones
    std::regex targetRx; // to be used for regexed match zones
...
*/
typedef struct {
    std::vector<whitelist_location_t> whitelistLocations;
    ...
    std::string name; // hash key [#]URI#VARNAME
    std::vector<int> ids;
} whitelist_rule_t;
...
typedef struct {
    ...
    */
    ...
    bool negative = false;
    std::vector<custom_rule_location_t> customLocations;
} basic_rule_t;
...
typedef struct {
    ...
    unsigned long id;
    std::string logMsg; // a specific log message
    /* List of scores increased on rule match. */
    std::vector<std::pair<std::string, unsigned long>> scores;
    rule_action_t action = ALLOW;
...
extern std::vector<http_rule_t> getRules;
extern std::vector<http_rule_t> bodyRules;
extern std::vector<http_rule_t> rawBodyRules;
extern std::vector<http_rule_t> headerRules;
extern std::vector<http_rule_t> genericRules; // URL

class RuleParser {
private:
    std::vector<http_rule_t> whitelistRules; // raw array of whitelist rules
    bool isRuleWhitelistedRx(const http_rule_t &rule, const std::string uri, const std::string &name, MATCH_ZONE zone,
                             bool targetName);
    bool isWhitelistAdapted(whitelist_rule_t &wlrule, const std::string &name, MATCH_ZONE zone, const http_rule_t &rule,
                            MATCH_TYPE type, bool targetName);

public:
    std::unordered_map<std::string, check_rule_t> checkRules;

    std::vector<whitelist_rule_t> tmpWlr; // raw array of transformed whitelists
    std::vector<http_rule_t> rxMzWlr; // raw array of regex-mz whitelists

    std::unordered_map<std::string, whitelist_rule_t> wlUrlHash; // hash table of whitelisted URL rules
    std::unordered_map<std::string, whitelist_rule_t> wlArgsHash; // hash table of whitelisted ARGS rules
    std::unordered_map<std::string, whitelist_rule_t> wlBodyHash; // hash table of whitelisted BODY rules
    std::unordered_map<std::string, whitelist_rule_t> wlHeadersHash; // hash table of whitelisted HEADERS rules
    std::vector<http_rule_t> disabled_rules; // rules that are globally disabled in one location
    ...
    RuleParser();
    static unsigned int parseMainRules(std::vector<std::string> &ruleLines, std::string errorMsg);
    void parseCheckRule(std::vector<std::pair<std::string, std::string>> &rulesArray, std::string errorMsg);
    unsigned int parseBasicRules(std::vector<std::string> &ruleLines, std::string errorMsg);
    static void parseAction(std::string action, rule_action_t& rule_action);
    static void parseMatchZone(http_rule_t &rule, std::string &rawMatchZone, std::stringstream &err);
    ...
    bool checkIds(unsigned long matchId, const std::vector<int> &wlIds);
    bool findWlInHash(whitelist_rule_t &wlRule, const std::string &key, MATCH_ZONE zone);
    bool isRuleWhitelisted(const http_rule_t &rule, const std::string& uri, const std::string &name, MATCH_ZONE zone,
                           bool targetName);
};


#endif //LIBDEFIANCE_RULEPARSER_H
