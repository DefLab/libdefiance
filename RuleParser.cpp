...
std::vector<http_rule_t> getRules;
std::vector<http_rule_t> bodyRules;
std::vector<http_rule_t> rawBodyRules;
std::vector<http_rule_t> headerRules;
std::vector<http_rule_t> genericRules; // URL
...
unsigned int RuleParser::parseMainRules(std::vector<std::string> &ruleLines, std::string errorMsg) {
    getRules.clear();
    bodyRules.clear();
    rawBodyRules.clear();
    headerRules.clear();
    ...
    for (std::string &ruleLine : ruleLines) {
        bool error = false;
        ...
        std::vector<std::string> ruleParts = Util::parseRawDirective(ruleLine);
        for (const std::string &rulePart : ruleParts) {
            if (rulePart == "negative") {
                ...
                std::string rx = rulePart.substr(3);
                std::transform(rx.begin(), rx.end(), rx.begin(), tolower);
                try {
                    rule.br.rx = std::regex(rx, std::regex::optimize);
                } catch (std::regex_error &e) {
                    err << "rx:" << rx << " " << parseCode(e.code()) << std::endl;
                    error = true;
                }
                ...
                std::string rawMatchZone = rulePart.substr(3);
                parseMatchZone(rule, rawMatchZone, err);
            } else if (rulePart.substr(0, 2) == "s:") {
                std::string score = rulePart.substr(2);
                std::vector<std::string> scores = Util::split(score, ',');
                DEBUG_CONF_MR("score=[");
                for (const std::string &sc : scores) {
                    if (sc.front() == '$') { // $SCORE
                        std::pair<std::string, std::string> scorepair = Util::splitAtFirst(sc, ":");
        ...
        if (!error)
            ...
            err << "MainRule #" << rule.id << " skipped" << std::endl;
        DEBUG_CONF_MR(endl);
    }
    errorMsg = err.str();
    ruleLines.clear();
...
void RuleParser::parseCheckRule(std::vector<std::pair<std::string, std::string>> &rulesArray, std::string errorMsg) {
    std::stringstream err;
    for (const std::pair<std::string, std::string> &rule : rulesArray) {
        const std::string &equation = rule.first;
        const std::string &action = rule.second;

        DEBUG_CONF_CR("CheckRule ");
        check_rule_t chkrule;
        std::vector<std::string> eqParts = Util::split(equation, ' ');

        std::string tag =
                (std::basic_string<char, std::char_traits<char>, std::allocator<char>> &&) Util::rtrim(eqParts[0]);
        DEBUG_CONF_CR(tag << " ");
        ...
        try {
            ...
            err << e.what() << " cannot convert " << eqParts[2] << " to integer" << std::endl;
            continue;
        }
...
unsigned int RuleParser::parseBasicRules(std::vector<std::string> &ruleLines, std::string errorMsg) {
    unsigned int ruleCount = 0;
    std::stringstream err;
    for (std::string &ruleLine : ruleLines) {
        ...
        for (const std::string &rulePart : ruleParts) {
            if (rulePart.substr(0, 3) == "wl:") {
                std::string rawWhitelist = rulePart.substr(3);
...
void RuleParser::parseAction(std::string action, rule_action_t &rule_action) {
    if (action == "BLOCK") {
        rule_action = BLOCK;
...
void RuleParser::parseMatchZone(http_rule_t &rule, std::string &rawMatchZone, std::stringstream &err) {
    std::vector<std::string> matchZones = Util::split(rawMatchZone, '|');
    for (const std::string &mz : matchZones) {
        if (mz[0] != '$') {
            if (mz == "ARGS") {
                rule.br.argsMz = true;
                DEBUG_CONF_MZ("ARGS ");
            ...
            std::pair<std::string, std::string> cmz = Util::splitAtFirst(mz, ":");
            ...
            if (!rule.br.rxMz) { // std::string MatchZone
                ...
                try {
                    customRule.targetRx = std::regex(cmz.second, std::regex::optimize);
                } catch (std::regex_error &e) {
                    err << "regex_error: " << parseCode(e.code()) << std::endl;
                    continue;
                }
    ...
    for (http_rule_t &curr_r : whitelistRules) {
        ...
        */
        if (curr_r.br.rxMz) {
             ...
             * Not necessary ! We supports it ;) (enhancement)
             */
...
bool RuleParser::checkIds(unsigned long matchId, const std::vector<int> &wlIds) {
    bool negative = false;
...
bool RuleParser::isWhitelistAdapted(whitelist_rule_t &wlrule, const std::string &name, MATCH_ZONE zone,
...
bool RuleParser::isRuleWhitelisted(const http_rule_t &rule, const std::string &uri, const std::string &name, MATCH_ZONE zone,
                                   bool targetName) {
    ...
    if (!wlUrlHash.empty()) {
        ...
        std::string hashname = std::string(uri);
        ...
        std::unordered_map<std::string, whitelist_rule_t>::const_iterator it = wlUrlHash.find(hashname);
        bool found = false;
        if (it != wlUrlHash.end()) {
            wlRule = it->second;
            found = true;
...
bool RuleParser::isRuleWhitelistedRx(const http_rule_t &rule, const std::string uri, const std::string &name,
...
bool RuleParser::findWlInHash(whitelist_rule_t &wlRule, const std::string &key, MATCH_ZONE zone) {
    if (zone == BODY || zone == FILE_EXT) {
        std::unordered_map<std::string, whitelist_rule_t>::const_iterator it = wlBodyHash.find(key);
        if (it != wlBodyHash.end()) {
            wlRule = it->second;
            return true;
        }
    } else if (zone == HEADERS) {
        std::unordered_map<std::string, whitelist_rule_t>::const_iterator it = wlHeadersHash.find(key);
        if (it != wlHeadersHash.end()) {
            wlRule = it->second;
            return true;
        }
    } else if (zone == URL) {
        std::unordered_map<std::string, whitelist_rule_t>::const_iterator it = wlUrlHash.find(key);
        if (it != wlUrlHash.end()) {
            wlRule = it->second;
            return true;
        }
    } else if (zone == ARGS) {
        std::unordered_map<std::string, whitelist_rule_t>::const_iterator it = wlArgsHash.find(key);
...
std::string RuleParser::parseCode(std::regex_constants::error_type etype) {
    switch (etype) {
