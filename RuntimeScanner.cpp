...
void RuntimeScanner::applyRuleMatch(const http_rule_t &rule, unsigned long nbMatch, MATCH_ZONE zone, 
                                    const std::string &name, const std::string &value, bool targetName) {
    if (logLevel >= LOG_LVL_NOTICE) {
        std::stringstream errlog;
        errlog << Util::formatLog(LOG_LVL_NOTICE, clientIp);
    ...
    if (learningJSONLogFile) {
        std::string fullZone = match_zones[zone];
        fullZone += (targetName ? "|NAME" : "");
        std::string matchInfoKey = name + "#" + fullZone;
...
void RuntimeScanner::applyCheckRule(const http_rule_t &rule, unsigned long nbMatch, const std::string &name,
                                    const std::string &value, MATCH_ZONE zone, bool targetName) {
    if (parser.isRuleWhitelisted(rule, uri, name, zone, targetName)) {
        if (logLevel >= LOG_LVL_NOTICE) {
            ...
            if (targetName)
                errlog << "in name ";
            errlog << "at " << match_zones[zone] << " " << name << ":" << value << KNRM << std::endl;
            streamToFile(errlog, errorLogFile);
        }
        return;
    }
    ...
    std::stringstream errlog;
    for (const std::pair<std::string, unsigned long> &tagScore : rule.scores) {
        bool matched = false;
...
bool RuntimeScanner::processRuleBuffer(const std::string &str, const http_rule_t &rl, unsigned long &nbMatch) {
...
//        nbMatch = Util::countSubstring(str.c_str(), str.size(), rl.br.str.c_str(), rl.br.str.size());
        nbMatch = Util::countSubstring(str.c_str(), rl.br.str.c_str(), rl.br.str.size());

        if (nbMatch > 0) {
            DEBUG_RUNTIME_PR("matched " << std::endl);
        ...
        nbMatch = (unsigned long) 
                distance(std::sregex_iterator(str.begin(), str.end(), rl.br.rx), std::sregex_iterator());
        if (nbMatch > 0) {
            DEBUG_RUNTIME_PR("matched " << std::endl);
            return !rl.br.negative;
        } else {
...
void RuntimeScanner::basestrRuleset(MATCH_ZONE zone, const std::string &name, const std::string &value,
    ...
    for (int i = 0; i < rules.size() && ((!block || learning) && !drop); i++) {
        ...
        */
        ...
        DEBUG_RUNTIME_BRS("done" << std::endl);
    }
}

void RuntimeScanner::checkLibInjection(MATCH_ZONE zone, const std::string &name, const std::string &value) {
    ...
    while (idx < len) {
        ...
        */
        ...
        if (filen_start) {
            ...
            nullbytes = Util::naxsi_unescape(&final_var);
            if (nullbytes > 0) {
                applyRuleMatch(parser.uncommonHexEncoding, 1, BODY, empty, empty, true);
            }
            nullbytes = Util::naxsi_unescape(&final_data);
            if (nullbytes > 0) {
                applyRuleMatch(parser.uncommonHexEncoding, 1, BODY, empty, empty, false);
            }

            /* here we got val name + val content !*/
            std::string finalVar = std::string((char *) final_var.data, final_var.len);
            std::string finalData = std::string((char *) final_data.data, final_data.len);
...
//            cerr << finalVar << ":" << finalData << std::endl;
 ...
 * splits the std::string into key/val std::pair
 * apply rules to key/val
 */
bool RuntimeScanner::splitUrlEncodedRuleset(char *str, const std::vector<http_rule_t> &rules, MATCH_ZONE zone) {
    str_t name, val;
    char *eq, *ev, *orig;
    ...
    while (str < (orig + full_len) && *str) {
        ...
        if (val.len) {
            nullbytes = Util::naxsi_unescape(&val);
...
void RuntimeScanner::addHeader(char *key, char *val) {
    ...
    if (k == "content-length") {
        ...
        if (v == "application/x-www-form-urlencoded") {
            ...
            rawContentType = std::string(val); // important: need to keep the case!
...
int RuntimeScanner::processHeaders() {
    // Scan headers
    for (std::pair<std::string, std::string> &headerPair : headers)
        basestrRuleset(HEADERS, headerPair.first, headerPair.second, headerRules);

    // Scan GET parameters
    for (const std::pair<std::string, std::string> &getPair : get)
...
void RuntimeScanner::streamToFile(const std::stringstream &ss, void *file) {
    if (!file) return;
...
void RuntimeScanner::writeLearningLog() {
    ...
    std::stringstream learninglog;
    learninglog << Util::naxsiTimeFmt() << " ";
    learninglog << "[error] ";
    learninglog << pid << "#";
    ...
    learninglog << std::endl;
    streamToFile(learninglog, learningLogFile);
}

void RuntimeScanner::writeExtensiveLog(const http_rule_t &rule, MATCH_ZONE zone, const std::string &name,
                                       const std::string &value, bool targetName) {
    std::stringstream extensivelog;
    extensivelog << Util::naxsiTimeFmt() << " ";
    extensivelog << "[error] ";
    extensivelog << pid << "#";
...
void RuntimeScanner::writeJSONLearningLog() {
    ...
    jsonlog << "\"scores\":{";
    for (const auto &match : matchScores) {
        std::string scoreName = match.first.substr(1, match.first.length() - 1);
    ...
    for (const auto &matchInfoPair : matchInfos) {
        ...
        if (!matchInfo.varname.empty())
            jsonlog << ",\"var_name\":\"" << Util::escapeQuotes(matchInfo.varname) << "\"";
        if (extensiveLearning && !matchInfo.content.empty())
            jsonlog << ",\"content\":\"" << Util::escapeQuotes(matchInfo.content) << "\"";
        jsonlog << "},";
    }
